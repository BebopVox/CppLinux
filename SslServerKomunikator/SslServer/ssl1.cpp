...
void ServerLoop(SSL *ssl, string ipAddress){
    ...
    // Command loop class, hash autocreate
    Loop l;    
    ...
    while(1){
        ...
        if(auth == 0){
            // Check credentials
            auth = l.Auth(buffer, ssl, SenderIP);    
            if(auth){        
                // Message
                char reply[] = "200|Authenticated\r\n";
                // Send message
                received = SSL_write(ssl, reply, strlen(reply));
                // get error
                end = sslError(ssl, received);                
                ...
                // Message
                char reply[] = "600|Authentication error\r\n";
                // Send message
                received = SSL_write(ssl, reply, strlen(reply));
                // get error
                end = sslError(ssl, received);
...
int main(){
    ...
    // get MX records
    vector<string> mx = sm.getMX("hello@breakermind.com",0,0);
    ...
    while(true){
        for (int i = 0; i < mx.size(); i++){
            ...
            // send email
            vector<string> files;
            // cout << sm.Send("hello@breakermind.com", "nanomoow@gmail.com", "hello@breakermind.com", "Smtp client test", "<h1>Smtp test</h1>", "<h1>Smtp test</h1>", files) << endl;
        }
        sleep(1);
    }
